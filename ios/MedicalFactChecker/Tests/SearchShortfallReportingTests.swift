...
final class SearchShortfallReportingTests: XCTestCase {
    ...
    /// A report whose text was assembled with these shortfalls, and which
    /// records them as a report generated by the workflow does (#284).
    private func report(shortfalls: [RetrievalShortfall]) throws -> EvidenceReport {
        EvidenceReport(
            verdict: .supported,
            summary: "A summary.",
            ...
            uniqueSourceCount: 1,
            documentsReviewed: 1,
            searchShortfallsRecord: try SearchFailureReporting.reportRecord(for: shortfalls)
        )
    }
    ...
    func testTheReportOpensWithTheNoticeAndRecordsTheGap() throws {
        let text = try report(shortfalls: [pubMedRateLimited]).fullReport
    ...
    func testACompleteSearchsReportIsUnchanged() throws {
        let complete = try report(shortfalls: [])
    ...
    func testTheNoticeIsDrawnBeforeTheVerdict() throws {
        let incomplete = try report(shortfalls: [pubMedRateLimited])
    ...
    func testTheSharedTextOpensWithTheNotice() throws {
        let shared = try report(shortfalls: [pubMedRateLimited]).plainTextReport
