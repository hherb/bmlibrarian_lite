...
- **The report keeps its own record.** `EvidenceReport`'s private
  ...
  which this app wrote, notice included — is read instead. A record that is
  present but damaged reports as incomplete, saying it cannot tell the reader
  what is missing, and nothing is taken off its text (#284).
