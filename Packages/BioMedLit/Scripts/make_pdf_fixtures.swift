...
//
// Not byte-reproducible: CGContext embeds a run-varying /ID and, for the two
// password-protected files, fresh encryption key material on every run, so
// re-running this against an unchanged fixture set still diffs all five
// files. The diff is spurious — semantically equivalent output, and the
// regenerated set passes the same tests — read it as such rather than as
...
// whole point of three of these five files is that PDFKit can get the text
// back out.
import CoreGraphics
import CoreText
import Foundation
